...
export default function HydraPage() {
  ...
  const totalCommands = hydraCategories.reduce(
    (acc, cat) => acc + cat.commands.length, 0
  )

  return (
    <div className="min-h-screen bg-background">
      <PageTitle title="Hydra — Login Bruteforcer" />
      <MainSidebar />
      <main className="lg:pl-64">
        ...
        <div className="relative overflow-hidden border-b border-border bg-gradient-to-br from-destructive/10 via-background to-orange-500/5">
          <div className="relative px-6 py-12 text-center lg:py-16">
            ...
            <h1 className="text-3xl font-bold text-foreground lg:text-4xl text-balance">Hydra — Password Bruteforcer</h1>
            <p className="mx-auto mt-3 max-w-2xl text-muted-foreground text-pretty">
              Fast online password brute-forcing tool supporting 50+ protocols including SSH, FTP, HTTP forms, databases, and more.
            </p>
            <div className="mt-6 flex flex-wrap items-center justify-center gap-3">
              <span className="rounded-full bg-destructive/10 px-4 py-2 text-sm font-medium text-destructive">{hydraCategories.length} Categories</span>
              ...
              <UsageGuide guide={getToolById('hydra')!} />
            </div>
          </div>
        </div>
                ...
                return (
                  <button key={cat.category} onClick={() => scrollToSection(sectionId)}
                    className={cn("flex shrink-0 items-center gap-2 rounded-lg px-3 py-2 text-sm font-medium transition-colors",
                      activeCategory === sectionId ? "bg-destructive text-white" : "text-muted-foreground hover:bg-muted hover:text-foreground")}>
        ...
        <div className="mx-auto max-w-5xl space-y-12 p-6">
          {hydraCategories.map((category, idx) => {
            ...
            return (
              <section key={idx} id={sectionId} className="scroll-mt-20">
                <div className="mb-6 flex items-center gap-3">
                  <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-destructive/10 text-destructive"><Terminal className="h-5 w-5" /></div>
                  <div>
                    <span className="text-xs font-medium text-destructive">Phase {idx + 1}</span>
          ...
          <section className="scroll-mt-20">
            <div className="mb-6 flex items-center gap-3">
              <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-destructive/10 text-destructive"><ExternalLink className="h-5 w-5" /></div>
              <div>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {hydraTools.map((tool) => (
                <a key={tool.name} href={tool.url} target="_blank" rel="noopener noreferrer"
                  ...
                  <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-md bg-destructive/10 text-destructive"><ExternalLink className="h-4 w-4" /></div>
                  <div className="min-w-0 flex-1">
                    <div className="font-medium text-foreground group-hover:text-destructive">{tool.name}</div>
