...
import { sqlmapCategories, sqlmapTools } from "@/lib/sqlmap-data"
import {
  Database,
  Terminal,
  ChevronRight,
...
export default function SQLMapPage() {
  ...
  const totalCommands = sqlmapCategories.reduce(
  ...
  return (
    <div className="min-h-screen bg-background">
      <PageTitle title="SQLMap — SQL Injection Automation" />
      <MainSidebar />
      ...
      <main className="lg:pl-64">
        ...
        <div className="border-b border-border bg-card/50">
          <div className="mx-auto max-w-5xl px-6 py-3">
            <nav className="flex items-center gap-2 text-sm text-muted-foreground">
              ...
              <ChevronRight className="h-4 w-4" />
              <span className="text-foreground">SQLMap</span>
            </nav>
          </div>
        </div>
        ...
        <div className="relative overflow-hidden border-b border-border bg-gradient-to-br from-destructive/10 via-background to-orange-500/5">
          <div className="relative px-6 py-12 text-center lg:py-16">
            ...
            <p className="mx-auto mt-3 max-w-2xl text-muted-foreground text-pretty">
              Automatic SQL injection detection and exploitation tool. Enumerate databases, dump tables, and gain OS shell access.
            </p>
            <div className="mt-6 flex flex-wrap items-center justify-center gap-3">
              <span className="rounded-full bg-destructive/10 px-4 py-2 text-sm font-medium text-destructive">
                {sqlmapCategories.length} Categories
              </span>
        ...
        <div className="mx-auto max-w-5xl space-y-12 p-6">
          {sqlmapCategories.map((category, idx) => {
            ...
            return (
              <section key={idx} id={sectionId} className="scroll-mt-20">
                ...
                <CommandList commands={category.commands} pageTitle="SQLMap" pageSize={15} />
              </section>
            )
          })}
