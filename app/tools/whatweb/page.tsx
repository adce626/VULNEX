...
import { whatwebCategories, whatwebTools } from "@/lib/whatweb-data"
import {
  Globe,
  Terminal,
  ChevronRight,
...
export default function WhatWebPage() {
  ...
  const totalCommands = whatwebCategories.reduce(
  ...
  return (
    <div className="min-h-screen bg-background">
      <PageTitle title="WhatWeb — Fingerprinting" />
      <MainSidebar />
      ...
      <main className="lg:pl-64">
        ...
        <div className="border-b border-border bg-card/50">
          <div className="mx-auto max-w-5xl px-6 py-3">
            <nav className="flex items-center gap-2 text-sm text-muted-foreground">
              ...
              <ChevronRight className="h-4 w-4" />
              <span className="text-foreground">WhatWeb</span>
            </nav>
          </div>
        </div>

        {/* Hero Section */}
        <div className="relative overflow-hidden border-b border-border bg-gradient-to-br from-emerald-500/10 via-background to-teal-500/5">
          <div className="relative px-6 py-12 text-center lg:py-16">
            ...
            <div className="mt-6 flex flex-wrap items-center justify-center gap-3">
              <span className="rounded-full bg-emerald-500/10 px-4 py-2 text-sm font-medium text-emerald-500">
                {whatwebCategories.length} Categories
              </span>
              ...
              <UsageGuide guide={getToolById('whatweb')!} />
            </div>
          </div>
        </div>
        ...
        <div className="mx-auto max-w-5xl space-y-12 p-6">
          {whatwebCategories.map((category, idx) => {
            ...
            return (
              <section key={idx} id={sectionId} className="scroll-mt-20">
                ...
                <CommandList commands={category.commands} pageTitle="WhatWeb" pageSize={15} />
              </section>
            )
          })}
          ...
          <section className="scroll-mt-20">
            <div className="mb-6 flex items-center gap-3">
              ...
              <div>
                <span className="text-xs font-medium text-emerald-500">Tools</span>
                <h2 className="text-2xl font-bold text-foreground">
                  Tools & Resources
                </h2>
              </div>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {whatwebTools.map((tool) => (
                <a
                  key={tool.name}
                  href={tool.url}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="group flex items-start gap-3 rounded-lg border border-border bg-card p-4 transition-all hover:border-emerald-500/50 hover:shadow-lg hover:shadow-emerald-500/5"
                >
                  <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-md bg-emerald-500/10 text-emerald-500">
                    <ExternalLink className="h-4 w-4" />
                  </div>
                  <div className="min-w-0 flex-1">
                    <div className="font-medium text-foreground group-hover:text-emerald-500">
                      {tool.name}
                    </div>
