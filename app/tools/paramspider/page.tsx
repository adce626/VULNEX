...
export default function ParamSpiderPage() {
  ...
  const totalCommands = paramSpiderCategories.reduce(
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="lg:pl-64">
        ...
        <div className="relative overflow-hidden border-b border-border bg-gradient-to-br from-primary/10 via-background to-accent/5">
          <div className="relative px-6 py-12 text-center lg:py-16">
            ...
            <h1 className="text-3xl font-bold text-foreground lg:text-4xl text-balance">
              ParamSpider — Passive Parameter Extraction
            </h1>
            <p className="mx-auto mt-3 max-w-2xl text-muted-foreground text-pretty">
              Extract URLs with parameters from Wayback Machine and Common Crawl archives — completely passive, no direct interaction with the target.
            </p>
            <div className="mt-6 flex flex-wrap items-center justify-center gap-3">
              <span className="rounded-full bg-primary/10 px-4 py-2 text-sm font-medium text-primary">
                {paramSpiderCategories.length} Categories
              </span>
              ...
              <UsageGuide guide={getToolById('paramspider')!} />
            </div>
          </div>
        </div>
        ...
        <div className="mx-auto max-w-5xl space-y-12 p-6">
          {paramSpiderCategories.map((category, idx) => {
