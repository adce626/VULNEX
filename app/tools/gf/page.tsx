...
import { gfCategories, gfTools } from "@/lib/gf-data"
import {
  Filter,
  Terminal,
  ChevronRight,
...
export default function GFPage() {
  ...
  const totalCommands = gfCategories.reduce(
  ...
  return (
    <div className="min-h-screen bg-background">
      <PageTitle title="gf — Pattern Matching" />
      <MainSidebar />
        ...
        <div className="relative overflow-hidden border-b border-border bg-gradient-to-br from-primary/10 via-background to-accent/5">
          <div className="relative px-6 py-12 text-center lg:py-16">
            ...
            <h1 className="text-3xl font-bold text-foreground lg:text-4xl text-balance">
              gf — URL Filter by Vulnerability Type
            </h1>
            <p className="mx-auto mt-3 max-w-2xl text-muted-foreground text-pretty">
              Filter collected URLs by vulnerability patterns for efficient triaging. Created by Tomnomnom — pipe URL lists into gf and extract XSS, SSRF, SQLi, and more.
            </p>
            <div className="mt-6 flex flex-wrap items-center justify-center gap-3">
              <span className="rounded-full bg-primary/10 px-4 py-2 text-sm font-medium text-primary">
                {gfCategories.length} Categories
              </span>
              ...
              <UsageGuide guide={getToolById('gf')!} />
            </div>
          </div>
        </div>
          ...
          {gfCategories.map((category, idx) => {
            ...
            return (
              <section key={idx} id={sectionId} className="scroll-mt-20">
                ...
                <CommandList commands={category.commands} pageTitle="gf" pageSize={15} />
              </section>
            )
          })}
