...
export default function FastXSSPage() {
  ...
  const totalCommands = fastXSSCategories.reduce((acc, cat) => acc + cat.commands.length, 0)

  return (
    <div className="min-h-screen bg-background">
      <PageTitle title="Fast XSS Pipeline" />
      <MainSidebar />
      <main className="lg:pl-64">
        ...
        <div className="relative overflow-hidden border-b border-border bg-gradient-to-br from-red-500/10 via-background to-orange-500/5">
          <div className="relative px-6 py-12 text-center lg:py-16">
            ...
            <h1 className="text-3xl font-bold text-foreground lg:text-4xl text-balance">Fast XSS</h1>
            <p className="mx-auto mt-3 max-w-2xl text-muted-foreground text-pretty">
              Fast and efficient XSS detection pipeline — GAU → gf → URO → Gxss → kxss → Loxs/Dalfox. Automate URL discovery, filter reflections, test payloads, and generate reports.
            </p>
            <div className="mt-6 flex flex-wrap items-center justify-center gap-3">
              <span className="rounded-full bg-red-500/10 px-4 py-2 text-sm font-medium text-red-500">{fastXSSCategories.length} Categories</span>
        ...
        <div className="mx-auto max-w-5xl space-y-12 p-6">
          {fastXSSCategories.map((category, idx) => {
            ...
            return (
              <section key={idx} id={sectionId} className="scroll-mt-20">
                <div className="mb-6 flex items-center gap-3">
                  ...
                  <div>
                    <span className="text-xs font-medium text-red-500">{idx === 0 ? "Intro" : `Phase ${idx}`}</span>
                    <h2 className="text-2xl font-bold text-foreground">{category.category}</h2>
                  </div>
                </div>
                <CommandList commands={category.commands} pageTitle="Fast XSS" pageSize={15} />
              </section>
            )
          })}
