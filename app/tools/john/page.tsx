...
import { johnCategories, johnTools } from "@/lib/john-data"
import {
  Key,
  Terminal,
  ChevronRight,
...
export default function JohnPage() {
  ...
  const totalCommands = johnCategories.reduce(
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="lg:pl-64">
        ...
        <div className="border-b border-border bg-card/50">
          <div className="mx-auto max-w-5xl px-6 py-3">
            <nav className="flex items-center gap-2 text-sm text-muted-foreground">
              ...
              <ChevronRight className="h-4 w-4" />
              <span className="text-foreground">John the Ripper</span>
            </nav>
          </div>
        </div>
          ...
          <div className="relative px-6 py-12 text-center lg:py-16">
            ...
            <div className="mt-6 flex flex-wrap items-center justify-center gap-3">
              <span className="rounded-full bg-red-500/10 px-4 py-2 text-sm font-medium text-red-500">
                {johnCategories.length} Categories
              </span>
              ...
              <UsageGuide guide={getToolById('john')!} />
            </div>
          </div>
        </div>
        ...
        <div className="mx-auto max-w-5xl space-y-12 p-6">
          {johnCategories.map((category, idx) => {
            ...
            return (
              <section key={idx} id={sectionId} className="scroll-mt-20">
                ...
                <CommandList commands={category.commands} pageTitle="John the Ripper" pageSize={15} />
              </section>
            )
          })}
