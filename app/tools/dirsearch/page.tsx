...
import { dirsearchCategories, dirsearchTools } from "@/lib/dirsearch-data"
import {
  FolderSearch,
  Terminal,
  ChevronRight,
...
export default function DirsearchPage() {
  ...
  const totalCommands = dirsearchCategories.reduce(
  ...
  return (
    <div className="min-h-screen bg-background">
      <PageTitle title="Dirsearch — Directory Bruteforce" />
      <MainSidebar />
      ...
      <main className="lg:pl-64">
        ...
        <div className="border-b border-border bg-card/50">
          <div className="mx-auto max-w-5xl px-6 py-3">
            <nav className="flex items-center gap-2 text-sm text-muted-foreground">
              ...
              <ChevronRight className="h-4 w-4" />
              <span className="text-foreground">Dirsearch</span>
            </nav>
          </div>
        </div>

        {/* Hero Section */}
        <div className="relative overflow-hidden border-b border-border bg-gradient-to-br from-orange-500/10 via-background to-amber-500/5">
          <div className="relative px-6 py-12 text-center lg:py-16">
            ...
            <div className="mt-6 flex flex-wrap items-center justify-center gap-3">
              <span className="rounded-full bg-orange-500/10 px-4 py-2 text-sm font-medium text-orange-500">
                {dirsearchCategories.length} Categories
              </span>
              ...
              <UsageGuide guide={getToolById('dirsearch')!} />
            </div>
          </div>
        </div>
        ...
        <div className="mx-auto max-w-5xl space-y-12 p-6">
          {dirsearchCategories.map((category, idx) => {
            ...
            return (
              <section key={idx} id={sectionId} className="scroll-mt-20">
                ...
                <CommandList commands={category.commands} pageTitle="Dirsearch" pageSize={15} />
              </section>
            )
          })}
          ...
          <section className="scroll-mt-20">
            <div className="mb-6 flex items-center gap-3">
              ...
              <div>
                <span className="text-xs font-medium text-orange-500">Tools</span>
                <h2 className="text-2xl font-bold text-foreground">
                  Tools & Resources
                </h2>
              </div>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {dirsearchTools.map((tool) => (
                <a
                  key={tool.name}
                  href={tool.url}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="group flex items-start gap-3 rounded-lg border border-border bg-card p-4 transition-all hover:border-orange-500/50 hover:shadow-lg hover:shadow-orange-500/5"
                >
                  <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-md bg-orange-500/10 text-orange-500">
                    <ExternalLink className="h-4 w-4" />
                  </div>
                  <div className="min-w-0 flex-1">
                    <div className="font-medium text-foreground group-hover:text-orange-500">
                      {tool.name}
                    </div>
