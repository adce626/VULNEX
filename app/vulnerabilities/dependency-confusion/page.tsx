...
import {
  dependencyConfusionCategories,
  dependencyConfusionTools,
  lastUpdated,
  pageDescription,
} from "@/lib/dependency-confusion-data"
import {
  Bug,
  Terminal,
  ChevronRight,
...
export default function DependencyConfusionPage() {
  ...
  return (
    <div className="min-h-screen bg-background">
      <PageTitle title="Dependency Confusion" />
      <MainSidebar />
      ...
      <main className="lg:pl-64">
        ...
        <div className="border-b border-border bg-card/50">
          <div className="mx-auto max-w-5xl px-6 py-3">
            <nav className="flex items-center gap-2 text-sm text-muted-foreground">
              ...
              <ChevronRight className="h-4 w-4" />
              <span className="text-foreground">Dependency Confusion</span>
            </nav>
          </div>
        </div>
        ...
        <div className="relative overflow-hidden border-b border-border bg-gradient-to-br from-primary/10 via-background to-accent/5">
          <div className="absolute inset-0 bg-[url('data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iNjAiIGhlaWdodD0iNjAiIHhtbG5zPSJodHRwOi8vd3d3LnczLm9yZy8yMDAwL3N2ZyI+PGRlZnM+PHBhdHRlcm4IGlkPSJncmlkIiB3aWR0PSIxIiBoZWlnaHQ9IjEiIGZpbGw9Im5vbmUiIHN0cm9rZT0icmRpYSgyNTUsMjU1LDI1NSwwLjAzKSIgc3Ryb2tlLXdpZHRoPSIxIi8+PC9zdmc+')] opacity-50" />
        ...
        <div className="sticky top-0 z-20 border-b border-border bg-card/95 backdrop-blur-sm">
          <div className="mx-auto max-w-5xl px-6">
            <div className="flex gap-1 overflow-x-auto py-3 scrollbar-hide">
              {dependencyConfusionCategories.map((cat) => {
        ...
        <div className="mx-auto max-w-5xl space-y-12 p-6">
          {dependencyConfusionCategories.map((category, idx) => {
          ...
          <section className="scroll-mt-20">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {dependencyConfusionTools.map((tool) => (
                <a
                  key={tool.name}
                  href={tool.url}
                  target="_blank"
          ...
          <footer className="border-t border-border pt-8 text-center">
            <p className="text-sm text-muted-foreground">
              This guide is for ethical use and authorized penetration testing only. Always follow responsible disclosure.
            </p>
          </footer>
        </div>
      </main>
