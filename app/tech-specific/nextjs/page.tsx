...
import { nextjsSteps } from "@/lib/nextjs-data"
import {
  ChevronLeft,
  Eye,
  ExternalLink,
...
export default function NextjsPage() {
  ...
  const totalCommands = nextjsSteps.reduce(
    (acc, step) => acc + step.commands.length,
    0
  )
  const totalTools = nextjsSteps.reduce(
      ...
      <main className="lg:pl-64">
        ...
        <div className="px-6 py-8">
          ...
          <div className="space-y-6">
            {nextjsSteps.map((step, stepIndex) => (
              <section
                key={step.id}
                id={step.id}
          ...
          <div className="mt-8 rounded-xl border border-destructive/30 bg-destructive/5 p-6">
            ...
            <ul className="mt-4 space-y-2 text-muted-foreground">
              <li className="flex items-start gap-2">
                <ArrowRight className="mt-1.5 h-4 w-4 shrink-0 text-destructive" />
                <span>Unauthorized access to protected routes and sensitive data</span>
              </li>
              <li className="flex items-start gap-2">
                <ArrowRight className="mt-1.5 h-4 w-4 shrink-0 text-destructive" />
                <span>Privilege escalation to admin-level access</span>
              </li>
              <li className="flex items-start gap-2">
                <ArrowRight className="mt-1.5 h-4 w-4 shrink-0 text-destructive" />
                <span>Potential complete application compromise</span>
              </li>
            </ul>
          </div>

          {/* Remediation */}
          <div className="mt-6 rounded-xl border border-primary/20 bg-gradient-to-br from-primary/5 to-accent/5 p-6">
            <div className="flex items-center gap-3">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              <div className="flex items-center gap-2 rounded-lg bg-background p-3">
                ...
                <span className="text-sm text-foreground">Next.js 15.x: 15.2.3+</span>
              </div>
              <div className="flex items-center gap-2 rounded-lg bg-background p-3">
                <Shield className="h-4 w-4 text-primary" />
                <span className="text-sm text-foreground">Next.js 14.x: 14.2.25+</span>
              </div>
              <div className="flex items-center gap-2 rounded-lg bg-background p-3">
                <Shield className="h-4 w-4 text-primary" />
                <span className="text-sm text-foreground">Next.js 13.x: 13.5.9+</span>
              </div>
              <div className="flex items-center gap-2 rounded-lg bg-background p-3">
                <Shield className="h-4 w-4 text-primary" />
                <span className="text-sm text-foreground">Next.js 12.x: 12.3.5+</span>
              </div>
            </div>
          </div>
