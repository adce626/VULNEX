...
export default function RateLimitBypassPage() {
  ...
  const totalCommands = rateLimitCategories.reduce((acc, cat) => acc + cat.commands.length, 0)
    ...
    <div className="min-h-screen bg-background">
      ...
      <main id="main-content" className="lg:pl-64">
        <div className="border-b border-border bg-card/50">
          <div className="mx-auto max-w-5xl px-6 py-3">
            <nav className="flex items-center gap-2 text-sm text-muted-foreground">
              ...
              <ChevronRight className="h-4 w-4" />
              <span className="text-foreground">Rate Limit Bypass</span>
            </nav>
          </div>
        </div>

        <div className="relative overflow-hidden border-b border-border bg-gradient-to-br from-purple-500/10 via-background to-violet-500/5">
          <div className="relative px-6 py-12 text-center lg:py-16">
            ...
            <h1 className="text-3xl font-bold text-foreground lg:text-4xl text-balance">Rate Limit Bypass</h1>
            ...
            <div className="mt-6 flex flex-wrap items-center justify-center gap-3">
              <span className="rounded-full bg-purple-500/10 px-4 py-2 text-sm font-medium text-purple-500">{rateLimitCategories.length} Categories</span>
                ...
                return (
                  <button key={cat.category} onClick={() => scrollToSection(sectionId)}
                    className={cn("flex shrink-0 items-center gap-2 rounded-lg px-3 py-2 text-sm font-medium transition-colors whitespace-nowrap",
                      activeCategory === sectionId ? "bg-purple-500 text-white" : "text-muted-foreground hover:bg-muted hover:text-foreground")}>
        ...
        <div className="mx-auto max-w-5xl space-y-12 p-6">
          {rateLimitCategories.map((category, idx) => {
            ...
            return (
              <section key={idx} id={sectionId} className="scroll-mt-20">
                <div className="mb-6 flex items-center gap-3">
                  <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-purple-500/10 text-purple-500"><Terminal className="h-5 w-5" /></div>
                  <div>
                    <span className="text-xs font-medium text-purple-500">{idx === 0 ? "Intro" : `Tech ${idx}`}</span>
          ...
          <section className="scroll-mt-20">
            <div className="mb-6 flex items-center gap-3">
              <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-purple-500/10 text-purple-500"><ExternalLink className="h-5 w-5" /></div>
              <div>
                <span className="text-xs font-medium text-purple-500">Tools</span>
                <h2 className="text-2xl font-bold text-foreground">Tools & Resources</h2>
              </div>
            </div>
            <div className="grid gap-3 sm:grid-cols-2">
              {rateLimitTools.map((tool) => (
                <a key={tool.name} href={tool.url} target="_blank" rel="noopener noreferrer"
                  className="group flex items-start gap-3 rounded-lg border border-border bg-card p-4 transition-all hover:border-purple-500/50 hover:shadow-lg hover:shadow-purple-500/5">
                  <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-md bg-purple-500/10 text-purple-500"><ExternalLink className="h-4 w-4" /></div>
                  <div className="min-w-0 flex-1">
                    <div className="font-medium text-foreground group-hover:text-purple-500">{tool.name}</div>
