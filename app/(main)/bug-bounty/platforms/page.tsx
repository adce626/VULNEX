...
import { platformChapters } from "@/lib/platforms-data"
import { Target, Copy, Check, Globe, ChevronRight, ExternalLink } from "lucide-react"
...
const navLinks = [
  ...
  { href: "/bug-bounty/recon-flow", label: "Recon Flow", color: "var(--bb-orange)", dot: "var(--bb-orange)" },
...
export default function PlatformsPage() {
  const [activeChapter, setActiveChapter] = useState(platformChapters[0].id)
  ...
  return (
    <div className="bug-bounty-realm">
      <PageTitle title="Platform Guides — Bug Bounty Platform Comparison & Setup" />
      ...
      <section className="relative overflow-hidden border-b px-6 pt-16 pb-12 text-center" style={{ borderColor: "var(--bb-border)" }}>
        ...
        <div className="relative mx-auto max-w-4xl">
          <div className="mx-auto mb-5 flex h-16 w-16 items-center justify-center rounded-2xl" style={{ background: "var(--bb-primary-glow)", border: "1px solid var(--bb-primary)" }}>
            <Globe className="h-8 w-8" style={{ color: "var(--bb-primary)" }} />
          </div>
        ...
        {platformChapters.map((chapter) => (
          <section key={chapter.id} id={chapter.id} className="mb-20 scroll-mt-28 last:mb-0">
            ...
            <div className="mt-10 rounded-xl border p-5" style={{ borderColor: "var(--bb-border)", background: "var(--bb-card)" }}>
              <p className="mb-4 text-xs font-bold tracking-widest" style={{ color: chapter.color }}>RESOURCES IN THIS CHAPTER</p>
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {chapter.tools.map((tool) => (
                  ...
                  >
                    ...
                    <p className="text-xs leading-relaxed" style={{ color: "var(--bb-text-secondary)" }}>{tool.desc}</p>
                  </a>
                ))}
              </div>
            </div>
      ...
      <footer className="border-t px-6 py-8 text-center" style={{ borderColor: "var(--bb-border)" }}>
        <Link href="/bug-bounty" className="inline-flex items-center gap-2 text-sm font-medium transition-colors hover:text-white" style={{ color: "var(--bb-text-muted)" }}>
          <svg className="h-4 w-4 rotate-180" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14M12 5l7 7-7 7" /></svg>
          Back to Bug Bounty
        </Link>
      </footer>
    </div>
